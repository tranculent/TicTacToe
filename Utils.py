...
import Modes.ComputerMode
import Constants

def drawBoard(board, root):
    for _ in range(9): # Create 9 buttons
        Button(root, text="", command=lambda: Modes.ComputerMode.getComputerMove(board, Constants.COMPUTER_LETTER)).pack(pady=10, padx=10, side=LEFT)
